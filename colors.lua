 local hex_chars = "0123456789abcdef" local epsilon = 0.0088564516 local kappa = 903.2962962 local refY = 1 local refU = 0.19783000664283 local refV = 0.46831999493879
 ...
 if (sub1 > epsilon) then sub2 = sub1 else
 sub2 = (l / kappa) end
 for i = 1, 3 do
 local m1 = m[i][1]
 local m2 = m[i][2]
 ...
 table.insert(result, {slope = (top1 / bottom), intercept = (top2 / bottom)}) end end


 return result end

 local function length_of_ray_until_intersect(theta, line)
 return (line.intercept / (math.sin(theta) - (line.slope * math.cos(theta)))) end


 local function max_safe_chroma_for_lh(l, h)
 local hrad = (((h / 360) * math.pi) * 2)
 local bounds = get_bounds(l) local min = 1.7976931348623e+308
 ...
 if (distance >= 0) then
 min = math.min(min, distance) else end end
 return min end


 local function y__3el(Y)
 if (Y <= epsilon) then return ((Y / refY) * kappa) else
 return ((116 * ((Y / refY) ^ 0.33333333333333)) - 16) end end

 local function l__3ey(L)
 if (L <= 8) then return ((refY * L) / kappa) else
 return (refY * (((L + 16) / 116) ^ 3)) end end

 local function from_linear(c)
 if (c <= 0.0031308) then return (12.92 * c) else
 return ((1.055 * (c ^ 0.41666666666667)) - 0.055) end end

 local function to_linear(c)
 if (c > 0.04045) then return (((c + 0.055) / 1.055) ^ 2.4) else
 return (c / 12.92) end end

 local function dot_product(a, b) local sum = 0
 ...
 local H = nil
 if (C < 1e-08) then H = 0 else


 H = ((math.atan2(V, U) * 180) / 3.1415926535898)

 if (H < 0) then
 H = (360 + H) else end end
 return {L, C, H} end
 ...
 return {L, (math.cos(Hrad) * C), (math.sin(Hrad) * C)} end
 ...
 var_u = (var_u / divider)
 var_v = (var_v / divider) else var_u = 0 var_v = 0 end
 ...
 local rtn = {0, 0, 0}
 return rtn else end
 return {L, ((13 * L) * (var_u - refU)), ((13 * L) * (var_v - refV))} end
 ...
 return {X, Y, ((((9 * Y) - ((15 * var_v) * Y)) - (var_v * X)) / (3 * var_v))} end




 local function xyz__3ergb(tuple)
 return {from_linear(dot_product(m[1], tuple)), from_linear(dot_product(m[2], tuple)), from_linear(dot_product(m[3], tuple))} end
 ...
 return {dot_product(minv[1], rgbl), dot_product(minv[2], rgbl), dot_product(minv[3], rgbl)} end
 ...
 do end (ret)[(i + 1)] = (((digit1 * 16) + digit2) / 255) end
 return ret end

 local function rgb__3ehex(tuple) local h = "#"
 ...
 h = (h .. string.sub(hex_chars, (digit2 + 1), (digit2 + 1))) end
 ...
 local rtn = {H, 0, 0}
 return rtn else end
 return {H, ((C / max_chroma) * 100), L} end
 ...
 local rtn = {0, 0, H}
 return rtn else end
 return {L, ((max_safe_chroma_for_lh(L, H) / 100) * S), H} end



 local function rgb__3elch(tuple)
 return luv__3elch(xyz__3eluv(rgb__3exyz(tuple))) end

 local function lch__3ergb(tuple)
 return xyz__3ergb(luv__3exyz(lch__3eluv(tuple))) end

 local function rgb__3ehsluv(tuple)
 return lch__3ehsluv(rgb__3elch(tuple)) end

 local function hsluv__3ergb(tuple)
 return lch__3ergb(hsluv__3elch(tuple)) end

 local function hex__3ehsluv(s)
 return rgb__3ehsluv(hex__3ergb(s)) end

 local function hsluv__3ehex(tuple)
 return rgb__3ehex(hsluv__3ergb(tuple)) end


 local function transform_h(c, f)
 return {f(c[1]), c[2], c[3]} end

 local function transform_s(c, f)
 return {c[1], f(c[2]), c[3]} end

 local function transform_l(c, f)
 return {c[1], c[2], f(c[3])} end

 local function linear_tween(start, stop)
 local function _16_(i) return (start + (i * (stop - start))) end return _16_ end
 ...
 local function _17_(i) return ((360 + math.deg((start + (delta * i)))) % 360) end return _17_ end
 ...
 return {h(ratio0), s(ratio0), l(ratio0)} end


 local function lighten(c, n)
 local l = linear_tween(c[3], 100)
 return {c[1], c[2], l(n)} end
 ...
 return {((n + c[1]) % 360), c[2], c[3]} end


 local function blend_hex(c1, c2, r)
 return hsluv__3ehex(blend_hsluv(hex__3ehsluv(c1), hex__3ehsluv(c2), r)) end


 local function lighten_hex(c, n)
 return hsluv__3ehex(lighten(hex__3ehsluv(c), n)) end


 local function darken_hex(c, n)
 return hsluv__3ehex(darken(hex__3ehsluv(c), n)) end


 local function saturate_hex(c, n)
 return hsluv__3ehex(saturate(hex__3ehsluv(c), n)) end


 local function desaturate_hex(c, n)
 return hsluv__3ehex(desaturate(hex__3ehsluv(c), n)) end


 local function rotate_hex(c, n)
 return hsluv__3ehex(rotate(hex__3ehsluv(c), n)) end
 ...
 ls = vim.list_extend(ls, {i}) end
 local function _18_(_241) return blend_hex(c1, c2, _241) end return vim.tbl_map(_18_, ls) end

 local function gradient_n(c1, c2, n)
 local ls = {}
 do local step = (1 / (n + 1))
 for i = 1, n, 1 do
 ls = vim.list_extend(ls, {(i * step)}) end end
 local function _19_(_241) return blend_hex(c1, c2, _241) end return vim.list_extend({c1}, vim.tbl_map(_19_, ls), {c2}) end
 ...
 base16_palette[name] = hex end
 return base16_palette end
